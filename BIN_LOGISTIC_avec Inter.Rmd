...
for (j in seq(0.3, 0.5, 0.05) ) {
  ...
  for (i in 1:10){
    ...
    # modele 
    BIN_LOG2.fit <- glm(Class_Binaire ~ ANNEE_POSE+AGE+VITESSE +UIC +TRAIN +Profil +TYPE_RAYON_COURBE + AGE:ANNEE_POSE + TYPE_RAYON_COURBE:VITESSE, data=training_set, family="binomial")
    BIN_LOG2.probs <- predict(BIN_LOG2.fit, testing_set, type = "response", allow.new.levels = TRUE)
    
    BIN_LOG2.pred <- rep("0", length(BIN_LOG2.probs))
    BIN_LOG2.pred[BIN_LOG2.probs > j] = "1"
    # matrice de confusion
    K <- table(BIN_LOG2.pred,testing_set$Class_Binaire)
    K <- addmargins(table(BIN_LOG2.pred,testing_set$Class_Binaire))
    # calcul du taux de bien classé, et des erreurs FP et FN
    CLASSE_OK<-(mean(BIN_LOG2.pred == testing_set$Class_Binaire))
    TFP <- K[2,1]/K[3,1]
    TFN <- K[1,2]/K[3,2]
...
Le modéle :
```{r}
BIN_LOG2.fit <- glm(Class_Binaire ~ ANNEE_POSE+AGE+VITESSE +UIC +TRAIN +Profil +TYPE_RAYON_COURBE+ AGE:ANNEE_POSE + TYPE_RAYON_COURBE:VITESSE, data=BASE_Bin_TRAIN, family="binomial")
summary(BIN_LOG2.fit)
```
Vérification de la significativité des variables :
```{r}
anova(BIN_LOG2.fit, test = "Chisq")
```
Elles sont toutes significatives.

Validation sur TEST :
```{r}
BIN_LOG2.probs <- predict(BIN_LOG2.fit, BASE_Bin_TEST, type = "response")
...
BIN_LOG2.pred <- rep("0", length(BIN_LOG2.probs))
BIN_LOG2.pred[BIN_LOG2.probs > .4] = "1"

# Calcul de la moyenne des bien predit sur le test :  
perf <- (mean(BIN_LOG2.pred == BASE_Bin_TEST$Class_Binaire) )

# Matrice de confusion associé au modèle
confusion <- table(BIN_LOG2.pred, BASE_Bin_TEST$Class_Binaire)

# Utilisation de la fonction `addmargins` pour ajouter les sommes de chaque ligne et colonne.
confusion <- addmargins(table(BIN_LOG2.pred, BASE_Bin_TEST$Class_Binaire))
confusion
