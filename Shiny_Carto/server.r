...
unique(Map_From_DB$TYPE_CLASSEMENT)
...
function(input, output, session)
  ...
  FiltreLigne <- reactive({
    ...
    Map_From_DB
    } else {subset(Map_From_DB, Map_From_DB$LIGNE_VOIE == input$LIGNE)}
  })
  ...
  pal <- colorFactor(c("green", "yellow", "orange","navy","red","black"), domain = c("E", "NR","O","X1","X2","S"))
  ...
  output$CarteLigne <- renderLeaflet({
    ...
    #FiltreLigne() <- subset(FiltreLigne(), FiltreLigne()$TYPE_CLASSEMENT %in% (input$TypeDefaut))
