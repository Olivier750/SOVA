...
Estime_Moins_Grave <- c()
  ...
  for (i in 1:10){
    ...
    # modele 
    MULTI_LOG2.fit <- multinom(TYPE_CLASSEMENT ~ ANNEE_POSE+AGE+VITESSE +UIC +TRAIN +Profil +TYPE_RAYON_COURBE + AGE:ANNEE_POSE + TYPE_RAYON_COURBE:VITESSE, data=training_set)
    MULTI_LOG2.pred <- predict(MULTI_LOG2.fit, newdata = testing_set)
    
    # matrice de confusion
    K <- table(MULTI_LOG2.pred, testing_set$TYPE_CLASSEMENT)
    K <- addmargins(table(MULTI_LOG2.pred,testing_set$TYPE_CLASSEMENT))
    
    # calcul du taux de bien classé, et des erreurs 
    CLASSE_OK<-(mean(MULTI_LOG2.pred == testing_set$TYPE_CLASSEMENT))
...
Le modéle :
```{r}
MULTI_LOG2.fit <- multinom(TYPE_CLASSEMENT ~ ANNEE_POSE+AGE+VITESSE +UIC +TRAIN +Profil +TYPE_RAYON_COURBE + AGE:ANNEE_POSE + TYPE_RAYON_COURBE:VITESSE, data=BASE_Multi_TRAIN)
summary(MULTI_LOG2.fit)
```
Validation sur TEST :
```{r}
MULTI_LOG2.pred <- predict(MULTI_LOG2.fit, newdata = BASE_Multi_TEST)

# Matrice de confusion :
K <- table(MULTI_LOG2.pred, BASE_Multi_TEST$TYPE_CLASSEMENT)
K <- addmargins(table(MULTI_LOG2.pred, BASE_Multi_TEST$TYPE_CLASSEMENT))

# calcul du taux de bien classé, et des erreurs 
CLASSE_OK<-(mean(MULTI_LOG2.pred == BASE_Multi_TEST$TYPE_CLASSEMENT))
# Calcul du taux d'estimé plus grave : 
TPG <- (K[3,1]+K[1,2]+K[3,2])/K[4,4]
# Calcul du taux d'estimé moins grave : 
TMG <- (K[2,1]+K[1,3]+K[2,3])/K[4,4]  
...
```{r}
MULTI_LOG2.fit.backaic <- step(MULTI_LOG2.fit)
```

Modele du step :
```{r}
summary(MULTI_LOG2.fit.backaic)
```

Validation sur TEST :
```{r}
MULTI_LOG2.pred.backaic <- predict(MULTI_LOG2.fit.backaic, newdata = BASE_Multi_TEST)

# Matrice de confusion :
K <- table(MULTI_LOG2.pred.backaic, BASE_Multi_TEST$TYPE_CLASSEMENT)
K <- addmargins(table(MULTI_LOG2.pred.backaic, BASE_Multi_TEST$TYPE_CLASSEMENT))

# calcul du taux de bien classé, et des erreurs 
CLASSE_OK<-(mean(MULTI_LOG2.pred.backaic == BASE_Multi_TEST$TYPE_CLASSEMENT))
# Calcul du taux d'estimé plus grave : 
TPG <- (K[3,1]+K[1,2]+K[3,2])/K[4,4]
# Calcul du taux d'estimé moins grave : 
TMG <- (K[2,1]+K[1,3]+K[2,3])/K[4,4]  
